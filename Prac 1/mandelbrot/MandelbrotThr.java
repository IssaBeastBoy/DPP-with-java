...
public class MandelbrotThr extends Applet
  ...
  { private int xsize;      // dimensions of window
    ...
    private ExecutorService pool = Executors.newFixedThreadPool(NUM_TASKS);
    ...
    private void generateImage()
      { startTime = System.currentTimeMillis();
        for (int i = 0; i < ysize; i += taskSize)
          { // Start thread
            Future<DisplalyInfo> store = pool.submit(new WorkerThread(i, i+taskSize));
            try
            {
              display(store.result, store.start_postion);
            }
            catch (Exception e)
            {
              e.getMessage();
    ...
    private static class WindowCloser extends WindowAdapter
      ...
      private class DisplalyInfo 
      {
        public byte[][] result;
        public int start;
        public DisplalyInfo(byte[][] result, int start){
          this.result = result;
          this.start = start;
        }
      }
    ...
    private class WorkerThread implements Callable<DisplalyInfo>
      { private int start;
        private int end;
        ...
        public DisplayInfo call()
          { double x, y, xx, a, b = y1, da = x2/xsize, db = y2/ysize;
            ...
            return new DisplayInfo(results, start);
          } // calculateMandelbrot
