  ...
  { private int xsize;      // dimensions of window
    ...
    private void generateImage()
      { startTime = System.currentTimeMillis();
        for (int i = 0; i < ysize; i += taskSize)
          { // Start thread
            Future<DisplayInfo> store = pool.submit(new WorkerThread(i, i+taskSize));
            try
            {
              display(store.get().result, store.get().start);
            }
            catch (Exception e)
            {
              e.getMessage();
    ...
    private static class WindowCloser extends WindowAdapter
      ...
      private class DisplayInfo 
      {
        public byte[][] result;
        public int start;
        public DisplayInfo(byte[][] result, int start){
          this.result = result;
          this.start = start;
        }
      }

    private class WorkerThread implements Callable<DisplayInfo>
      { private int start;
        private int end;
